...
if __name__ == "__main__":
    ...
    gz_process = subprocess.Popen(['gz', 'sim', '../worlds/example_swarm.sdf', '-r', '-s'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    time.sleep(15)

    #completed_process = subprocess.run('gz service -l', capture_output=True, shell=True)
    ...
    for d_z in range(0, 30, 5):
        ...
        timeout = 5.0               # Timeout in case something goes wrong
    ...
    subprocess.run(['pkill', '--full', 'gz sim server'])
    subprocess.run(['pkill', '--full', 'gz sim gui'])
